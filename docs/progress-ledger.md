...
- Active (2026-07-14): `feat/TASK-044-tracking-foundations` (base `c3a9ec1`) —
  plan Slices 0+A landed; `feat/TASK-043-school-command-ui` — schools UI
...
| 2026-07-12 | Upload-review batch: annotated preview, Gemini evaluator, court gate, trim button | Done (Cycle 23) | TASK-041 — diagnosis: pose data healthy (skeletons were live-overlay-only), MAX_REEL_SEC=59 capped reel at 3 rallies, shuttle_q 0.0 = teleport penalty flagging background court. Shipped: `annotated.mp4` baked shuttle+pose preview + Studio button; Gemini frame evaluator (main-court player count + keep ids, prunes tracks pre-camera, fail-open); `court_shuttle_gate` (background-court flight segments dropped by geometry); MAX_REEL_SEC 90; one-button whole-match trim (`/api/jobs/{id}/trim` → trimmed.mp4); rVFC frame-exact overlays; camera-follow fixed (targets resolved in displayed-crop space via toDisplayNorm); annotated-bake fixes (iter_frames pairs + read-only frombuffer→np.array); court gate redesigned lateral-only (never delete flight); RunPod deadline split queue-vs-exec (was abandoning COMPLETED GPU jobs→CPU fallback); shuttle_quality scored on FILTERED track consumers use; trim OOM→sequential encodes. **FINAL VERIFIED (GPU run, job adda60dbf93e): backend=runpod, 5 rallies, evaluator ok (bg boxes pruned), shuttle_q [1.0,0.91,0.81,0.87,0.84] mean 0.89 (was ~0), annotated.mp4+reel+trimmed all serving.** v=39, 153 tests |
| 2026-07-13 | Court player gate, action camera, audio rally veto | Deployed (Cycle 24) | TASK-042 — feet-in-quad court player gate (0 left-court boxes), id-churn-safe evaluator prune + revert guard, court-aware action camera (constant zoom ≤2.25, 1.55x on owner job), audio-impact rally veto (19 refined rallies), `ps-*` pose classes (glow CSS collision), drawn-court survives reprocess. GPU-verified on `1faaa5e4a02f`, v=40 |
| 2026-07-14 | Schools UI: five pre-HexOS screens | UI complete on branch | TASK-043 — `feat/TASK-043-school-command-ui` (web-next): School Today, Student Profile, Match Hub, Athlete Home, Progress Deep Dive + Schedule/Courts/Reports routes, shared shell, capture gates. Deployed vanilla client untouched |
| 2026-07-14 | Tracking foundations: cadence + health + pose sanitizer | On branch (Cycle 25) | TASK-044 — accepted plan Slices 0+A (`docs/reviews/2026-07-14-tracking-pose-segmentation-plan.md`): worker 180-frame cap → 1080 safety ceiling w/ per-rally `sampling` telemetry (60s rallies keep ~6 Hz, was 3 Hz; needs worker image rebuild); per-track health vectors (gap/coverage/conf per BoT-SORT id); `PUBLIC_TRACK_MAX_FRAMES` 1080; NEW `sanitize.py` before One-Euro (same-id teleports → `seg` breaks, body-relative joint gates, bone-surge rejection, `rejected` provenance — the 0.10→0.90→0.12 repro now rejects instead of smoothing to 0.7645); seg-keyed smoothing; `_stable_ids` slot-reuse distance gate; Studio pose interp 0.9→0.45s + no lerp across `seg`. 192 tests. Slices B–E queued in PRD |
