...
| P1 | In-play mask v1 + highlight score v2: fuse shuttle-flight ∪ audio peaks ∪ player kinetics per 0.25s bin with hysteresis; snap Gemini boundaries to serve impact/landing; split long rallies at ≥3s dead spans; rank highlights by in-play duration × (hit density + shot speed + audio prominence). Ships as analysis data first; trims renders only after bench gate (mask IoU ≥0.85, boundary MAE ≤0.5s) — design in `docs/reviews/2026-07-12-rally-break-inplay-audit.md` | Queued | TASK-040 |
