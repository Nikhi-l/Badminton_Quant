...
| P0 | (owner report 2026-07-14, job `713a86e5db5a` — 4 "players" in singles, juggling poses) Rebirth-era worker ids are confidence-rank aliases (31% impossible same-id steps) + pre-TASK-042 results display stored staff boxes forever: `_ids_from_worker` motion-coherence guard (incoherent ids → heuristic), court gate retro-applied at the public samplers (players+poses, corners threaded from `result.court`; bbox-bottom pose foot for gating; pose-only fail-open). Full repair of the stored reel = reprocess | Accept | TASK-045 (**on branch**, stacked on 044) |
| P1 | (owner request 2026-07-14) Court drawing flexibility: corners placeable OUTSIDE the frame (side-angle cameras cut endpoints off; validation widened to ±0.75, both pickers gain a dark clickable margin — Studio via 0.72 draw-zoom with exact inverse mapping) + upload-picker frame scrubber (court blocked by a player on the default frame). Homography/gates are plane geometry — extrapolated corners are first-class | Accept | TASK-046 (**on branch**, stacked on 045) |
| P1 | (owner request 2026-07-24) Replace the public root with the accepted dark/neon Baddy landing while preserving the existing uploader/gallery/Studio at `/create.html`; production-safe CTA targets, legacy root-hash redirects, original Studio/rally assets, isolated hashed React bundle | Accept | TASK-047 (**done + deployed 2026-07-24**, main `f43ed3f`) |
