...
def eval_tracknet(model, data_loader, param_dict):
    ...
    for step, (i, x, y, _, _) in enumerate(data_prob):
        x, y = x.float().to(_DEV), y.float().to(_DEV)
        with torch.no_grad():
            y_pred = model(x)
...
def eval_inpaintnet(model, data_loader, param_dict):
    ...
    for step, (i, coor_pred, coor, _, _, inpaint_mask) in enumerate(data_prob):
        coor_pred, coor, inpaint_mask = coor_pred.float().to(_DEV), coor.float().to(_DEV), inpaint_mask.float().to(_DEV)
    ...
    if inpaintnet is None:
        ...
        if param_dict['eval_mode'] == 'nonoverlap':
            ...
            for step, (i, coor_pred, coor, _, _, inpaint_mask) in enumerate(data_prob):
                ...
                with torch.no_grad():
                    coor_inpaint = inpaintnet(coor_pred.to(_DEV), inpaint_mask.to(_DEV)).detach().cpu()
...
if __name__ == '__main__':
    ...
    if args.tracknet_file:
        tracknet_ckpt = torch.load(args.tracknet_file, map_location=_DEV)
        param_dict['tracknet_seq_len'] = tracknet_ckpt['param_dict']['seq_len']
        param_dict['bg_mode'] = tracknet_ckpt['param_dict']['bg_mode']
        tracknet = get_model('TrackNet', seq_len=param_dict['tracknet_seq_len'], bg_mode=param_dict['bg_mode']).to(_DEV)
    ...
    if args.inpaintnet_file:
        inpaintnet_ckpt = torch.load(args.inpaintnet_file, map_location=_DEV)
        param_dict['inpaintnet_seq_len'] = inpaintnet_ckpt['param_dict']['seq_len']
        inpaintnet = get_model('InpaintNet').to(_DEV)
